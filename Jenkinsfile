            ...
            steps {
                withCredentials([string(credentialsId: 'github-cred', variable: 'GITHUB_TOKEN')]) {
                    sh '''
                        ...
                        sed -i "s/movies-front:1/movies-front:${IMAGE_TAG}/g" kubernetes/frontend-deployment.yaml
                        sed -i "s/movies-server:1/movies-server:${IMAGE_TAG}/g" kubernetes/server-deployment.yaml
                        git add .
